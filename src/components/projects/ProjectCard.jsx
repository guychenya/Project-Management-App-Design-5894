...
const { FiCalendar, FiUsers, FiDollarSign, FiTrendingUp, FiMoreHorizontal, FiEdit, FiTrash2 } = FiIcons;

const ProjectCard = ({ project, viewMode = 'grid', compact = false, index = 0 }) => {
  const { teamMembers, deleteProject } = useProject();
  ...
  return (
    <>
      <motion.div
        ...
        className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-200 task-card ${compact ? 'p-4' : 'p-6'}`}
      >
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <Link
              to={`/projects/${project.id}`}
              className={`font-semibold text-gray-900 dark:text-white hover:text-primary-600 dark:hover:text-primary-400 transition-colors duration-200 ${compact ? 'text-base' : 'text-lg'}`}
            >
              {project.name}
            </Link>
            {!compact && (
